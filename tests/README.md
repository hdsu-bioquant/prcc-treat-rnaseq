...
The synthetic wrapper intentionally does not use a site's production Snakemake profile. It
runs the same fixed two-core test at every site and binds its own repository root into
Apptainer, so it does not depend on site-specific container bind settings. On HPC systems it
can be launched from an allocated compute node if login-node execution is prohibited.
Environment-specific profile configuration is exercised by the realistic production-style
test below.
