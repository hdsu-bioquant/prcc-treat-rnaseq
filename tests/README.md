...
and runs the fixed tiny DAG locally with two cores. It explicitly binds the repository root
read-write into Apptainer, because every synthetic input, reference and output lives beneath
that directory. This keeps the qualification independent of site-specific storage bind paths
while allowing it to run from repositories located on shared or non-home filesystems.
